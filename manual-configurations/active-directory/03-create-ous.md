...
2. Right-click `lab.local` and create `Users`, `Computers`, `Servers`, and `Groups` OUs.
3. Under `Users`, create `Sales`, `IT`, `Finance`, and `HR` OUs.
4. Under `Computers`, create `Workstations` and `Kiosks` OUs.
5. Protect each OU from accidental deletion.
...
```powershell
Start-Transcript -Path C:\Logs\create-organizational-units.txt -Append
```

```powershell
New-ADOrganizationalUnit -Name 'Users' -Path 'DC=lab,DC=local' -ProtectedFromAccidentalDeletion $true
```
```powershell
New-ADOrganizationalUnit -Name 'Sales' -Path 'OU=Users,DC=lab,DC=local' -ProtectedFromAccidentalDeletion $true
```
```powershell
New-ADOrganizationalUnit -Name 'Workstations' -Path 'OU=Computers,DC=lab,DC=local' -ProtectedFromAccidentalDeletion $true
```
...
- **OU already exists:** Use `Get-ADOrganizationalUnit` first, then decide whether to reuse or rename it.
- **Accidental deletion protection blocks cleanup:** Disable protection intentionally with `Set-ADObject -ProtectedFromAccidentalDeletion $false` only after confirming the target.
...
Continue with [04-create-users-groups.md](04-create-users-groups.md).
