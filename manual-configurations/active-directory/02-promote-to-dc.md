...
1. Open Server Manager and select the notification flag.
2. Choose Promote this server to a domain controller.
3. Select Add a new forest.
4. Enter root domain name `lab.local`.
5. Choose forest and domain functional levels for Windows Server 2016 or later.
6. Enable DNS and Global Catalog.
7. Enter a Directory Services Restore Mode password from the lab password vault.
8. Review prerequisites and start promotion.
...
```powershell
Start-Transcript -Path C:\Logs\promote-server-to-domain-controller.txt -Append
```

```powershell
Install-ADDSForest -DomainName 'lab.local' -DomainNetbiosName 'LAB' -InstallDns -NoRebootOnCompletion:$false
```
...
- **Prerequisite check fails:** Review the warning text, confirm static IP, and verify the hostname is set to `DC01` before promotion.
- **DNS delegation warning:** In a single lab forest this warning is expected because there is no parent DNS zone to delegate from.
...
![Domain controller promotion](/screenshots/dc01.lab.local.png)
