...
Checked ADUC and confirmed `jsmith` was locked. Searched Security event `4740` on `DC01` and identified `CLIENT01` as the source. Confirmed the user recently changed password and had saved credentials on the laptop.
...
Unlocked the account with `Unlock-ADAccount -Identity jsmith`, reset the password using the approved temporary password workflow, required change at next logon, and cleared saved credentials with `rundll32.exe keymgr.dll, KRShowKeyMgr`.
...
Saved old credentials on the laptop repeatedly submitted the previous password after vacation.
